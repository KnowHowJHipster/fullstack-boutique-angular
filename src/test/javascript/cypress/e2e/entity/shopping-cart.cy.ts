...
describe('ShoppingCart e2e test', () => {
  ...
  // const shoppingCartSample = {"placedDate":"2024-06-03T17:54:40.790Z","status":"REFUNDED","totalPrice":10532.14,"paymentMethod":"IDEAL"};
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      ...
      body: {"gender":"OTHER","phone":"(332) 597-5064","addressLine1":"where punctually silly","addressLine2":"explode darling","city":"Wildermanbury","country":"Curacao"},
    }).then(({ body }) => {
  ...
  describe('new ShoppingCart page', () => {
    ...
    it.skip('should create an instance of ShoppingCart', () => {
      cy.get(`[data-cy="placedDate"]`).type('2024-06-04T06:03');
      cy.get(`[data-cy="placedDate"]`).blur();
      cy.get(`[data-cy="placedDate"]`).should('have.value', '2024-06-04T06:03');

      cy.get(`[data-cy="status"]`).select('REFUNDED');

      cy.get(`[data-cy="totalPrice"]`).type('26833.66');
      cy.get(`[data-cy="totalPrice"]`).should('have.value', '26833.66');

      cy.get(`[data-cy="paymentMethod"]`).select('IDEAL');

      cy.get(`[data-cy="paymentReference"]`).type('following');
      cy.get(`[data-cy="paymentReference"]`).should('have.value', 'following');
