...
describe('ProductOrder e2e test', () => {
  ...
  // const productOrderSample = {"quantity":25002,"totalPrice":951.91};
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      method: 'POST',
      url: '/api/products',
      body: {"name":"er","description":"weakly durian ugh","price":30339.61,"productSize":"XXL","image":"Li4vZmFrZS1kYXRhL2Jsb2IvaGlwc3Rlci5wbmc=","imageContentType":"unknown"},
    }).then(({ body }) => {
      product = body;
    });
    // create an instance at the required relationship entity:
    cy.authenticatedRequest({
      ...
      body: {"placedDate":"2024-06-04T10:33:38.271Z","status":"PAID","totalPrice":12488.28,"paymentMethod":"CREDIT_CARD","paymentReference":"call meh"},
    }).then(({ body }) => {
      shoppingCart = body;
    });
  });
  ...
  describe('new ProductOrder page', () => {
    ...
    it.skip('should create an instance of ProductOrder', () => {
      cy.get(`[data-cy="quantity"]`).type('11857');
      cy.get(`[data-cy="quantity"]`).should('have.value', '11857');

      cy.get(`[data-cy="totalPrice"]`).type('30832.55');
      cy.get(`[data-cy="totalPrice"]`).should('have.value', '30832.55');
