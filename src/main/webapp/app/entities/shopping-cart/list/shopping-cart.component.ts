...
import { EntityArrayResponseType, ShoppingCartService } from '../service/shopping-cart.service';
...
export class ShoppingCartComponent implements OnInit {
  ...
  trackId = (_index: number, item: IShoppingCart): number => this.shoppingCartService.getShoppingCartIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.shoppingCarts = this.refineData(dataFromBody);
  }

  protected refineData(data: IShoppingCart[]): IShoppingCart[] {
    const { predicate, order } = this.sortState();
    return predicate && order ? data.sort(this.sortService.startSort({ predicate, order })) : data;
  }

  protected fillComponentAttributesFromResponseBody(data: IShoppingCart[] | null): IShoppingCart[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.shoppingCartService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
