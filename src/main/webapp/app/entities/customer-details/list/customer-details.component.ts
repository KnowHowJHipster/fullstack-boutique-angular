...
import { EntityArrayResponseType, CustomerDetailsService } from '../service/customer-details.service';
...
export class CustomerDetailsComponent implements OnInit {
  ...
  trackId = (_index: number, item: ICustomerDetails): number => this.customerDetailsService.getCustomerDetailsIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.customerDetails = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: ICustomerDetails[] | null): ICustomerDetails[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.customerDetailsService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
